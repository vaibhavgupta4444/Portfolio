  ...
  return (
    <>
      <div className='fixed w-full md:w-0 md:h-[100vh] px-6 py-8 sm:px-4 sm:py-8 md:px-8 md:py-10 lg:px-10 lg:py-12 flex md:flex-col justify-between flex-row items-center z-50'>
        ...
        {!sidemenu && <motion.button whileTap={{opacity:0.5,scale:0.95}} transition={{type:'spring',duration:0.5}} onClick={() => setSidemenu(true)} className='block md:hidden hover:bg-gray-400/30 p-4'><i  className="fa-solid fa-bars hover:scale-105 transition-all duration-200"></i></motion.button>}
      </div>
      {/*-------------------------------------------------- small screen navbar-------------------------------------------- */}
      <div className={`flex flex-col items-center z-50 justify-between md:hidden fixed h-[100vh] w-full sidebar ${sidemenu ? 'top-0' : 'top-[-100vh]'} transition-all duration-500`}>
        <div className='flex justify-between items-center w-full px-6 py-8'>
          <a href='/' onClick={() => setSidemenu(false)}>
            <p className='text-3xl font-bold'>V</p>
          </a>
          <motion.button  whileTap={{scale:0.95,opacity:0.5}} transition={{type:'spring',duration:0.5}} onClick={() => setSidemenu(false)}  className='p-4 hover:bg-gray-400/30'><i className="fa-solid fa-xmark text-xl"></i></motion.button>
        </div>
